            ...
            @foreach($data as $muses)
                <div class="card h-50 mb-4" style="width: 19rem;">
                    <div class="d-flex justify-content-center">
                        <img src="{{ url($muses['image']) }}" class="card-img-top w-50" alt="...">
                    </div>
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-center">
                            ...
                            <form action="{{ route('cart.add') }}" method="post" class="m-1">
                                <input type="hidden" id="product_id-{{ $muses['id'] }}" name="product_id" value="{{ $muses['id'] }}">
                                <input type="number" id="quantity-{{ $muses['id'] }}" name="quantity" value="1" min="1" class="form-control" style="width: 70px; display: none;">
                                <button onclick="addToCart({{ $muses['id'] }})" type="button" class="btn btn-outline-dark border-2" data-bs-toggle="modal" data-bs-target="#staticBackdrop" style="font-size: 14px">
                                    В корзину
