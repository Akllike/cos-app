            ...
            @foreach($data as $gels)
                <div class="card h-100 mb-4" style="width: 19rem;">
                    <div class="d-flex justify-content-center">
                        <img src="{{ url($gels['image']) }}" class="card-img-top w-50" alt="...">
                    </div>
                    <div class="card-body">
                        ...
                        <p class="card-text text-secondary mt-1">{{$gels['price']}} руб. / {{ $gels['volume'] }} мл</p>
                        <div class="d-flex justify-content-center">
                            <a href="{{ url('catalog/gels') }}/{{ $gels['id'] }}" class="btn btn-dark border-2 m-1" style="font-size: 14px">Подробнее</a>
                            <form action="{{ route('cart.add') }}" method="post" class="m-1">
                                <input type="hidden" id="product_id-{{ $gels['id'] }}" name="product_id" value="{{ $gels['id'] }}">
                                <input type="number" id="quantity-{{ $gels['id'] }}" name="quantity" value="1" min="1" class="form-control" style="width: 70px; display: none;">
                                <button onclick="addToCart({{ $gels['id'] }})" type="button" class="btn btn-outline-dark border-2" data-bs-toggle="modal" data-bs-target="#staticBackdrop" style="font-size: 14px">
                                    В корзину
