                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-center">
                            ...
                            <form action="{{ route('cart.add') }}" method="post" class="m-1">
                                <input type="hidden" id="product_id-{{ $scrab['id'] }}" name="product_id" value="{{ $scrab['id'] }}">
                                <input type="number" id="quantity-{{ $scrab['id'] }}" name="quantity" value="1" min="1" class="form-control" style="width: 70px; display: none;">
                                <button onclick="addToCart({{ $scrab['id'] }})" type="button" class="btn btn-outline-dark border-2" data-bs-toggle="modal" data-bs-target="#staticBackdrop" style="font-size: 14px">
                                    В корзину
