                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-center">
                        ...
                        <form action="{{ route('cart.add') }}" method="post" class="m-1">
                            <input type="hidden" id="product_id-{{ $item['id'] }}" name="product_id" value="{{ $item['id'] }}">
                            <input type="number" id="quantity-{{ $item['id'] }}" name="quantity" value="1" min="1" class="form-control" style="width: 70px; display: none;">
                            <button onclick="addToCart({{ $item['id'] }})" data-id="{{ $item['id'] }}" type="button" class="btn btn-outline-dark border-2" data-bs-toggle="modal" data-bs-target="#staticBackdrop" style="font-size: 14px">
                                В корзину
